/**
 * Integration tests for conditional breakpoints in Python adapter
 *
...
async function startTestServer(): Promise<void> {
  ...
  client = new Client({
    name: 'conditional-bp-test-client',
    version: '0.1.0',
    capabilities: { tools: {} }
  });
  ...
  const logFilePath = path.resolve(currentDirName, '../../conditional_bp_test_server.log');
  try {
...
describe('Python Conditional Breakpoint Integration @requires-python', () => {
  let sessionId: string;
  const scriptPath = path.resolve('examples/python/conditional_loop.py');
  const breakpointLine = 7; // Line: value = i * 2
  ...
  it('should set conditional breakpoint and only stop when condition is true', async () => {
    ...
    const createResult = parseToolResult(
      await client.callTool({
        ...
        arguments: { language: 'python', name: 'conditionalBpTest' }
      })
    );
    ...
    // Note: Before debugging starts, conditionVerified may be undefined
    // because no proxy is running yet to validate the condition

    // 3. Start debugging (without stopOnEntry so we go straight to breakpoint)
  ...
  it('should include condition metadata in breakpoint response', async () => {
    ...
    const createResult = parseToolResult(
      await client.callTool({
        name: 'create_debug_session',
        arguments: { language: 'python', name: 'bpMetadataTest' }
      })
    );
